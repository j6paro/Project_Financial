...
def scraping_stock_data():
    ...
    df_stockdata["Year"] = df_stockdata["Date"].astype('str').str.split('-').str[0] # 연도 컬럼 생성

    # 전처리용 데이터프레임 만들기
    df_preprocessing = df_stockdata[df_stockdata.Dividends != 0] # Dividends가 0인 행 모두 삭제
    ...
    df_pivot_dividends = pd.pivot_table(df_preprocessing, values='Dividends', index='Year', aggfunc=np.sum) # 배당금 피벗 테이블 만들기
    df_pivot_price = pd.pivot_table(df_preprocessing, values='Close', index='Year', aggfunc=np.mean) # 종가 피벗 테이블 만들기

    # 종가 테이블, 배당금 테이블 join 하기
    df_merged = pd.merge(df_pivot_dividends, df_pivot_price, on='Year', how='inner') # 인덱스로 join 하기
    df_merged["Dividends yield rate"] = round((df_merged["Dividends"]/df_merged["Close"])*100, 1) # 배당률 컬럼 생성
    ...
    date_lastclose = str(df_stockdata['Date'].iloc[-1]).split()[0] # 가장 최근 종가 날짜
    price_last_close = df_stockdata['Close'].iloc[-1] # 가장 최근 종가
    date_last_dividends = str(df_preprocessing['Date'].iloc[-1]) # 마지막 배당일

    if str(now).split()[0][:7] == date_last_dividends[:7]: # 만약 이번달에 배당이 나옴 -> 배당금합 = 11개월 전~이번달 배당합
        start_date = str(now.year-1) + '-' + str(now.month+1).zfill(2) + '-01' # 시작일을 11개월 전 1일로
        df_trailing_dividends = df_preprocessing[df_preprocessing['Date'].bewteen(start_date, now_transform)] # 최근 1년 데이터프레임
        trailing_dividends_yield_rate = (df_trailing_dividends['Dividends'].sum / price_last_close) * 100 # 최근 1년 배당률 구하기
    else:
        start_date = str(now.year-1) + '-' + str(now.month).zfill(2) + '-01' # 시작일을 11개월 전 1일로
        df_trailing_dividends = df_preprocessing[df_preprocessing['Date'].between(start_date, now_transform)] # 최근 1년 데이터프레임
        trailing_dividends_yield_rate = (df_trailing_dividends['Dividends'].sum() / price_last_close) * 100 # 최근 1년 배당률 구하기
    current_dividends_yield_rate.set(f"마지막 종가일({date_lastclose})기준 배당률 : {round(trailing_dividends_yield_rate,1)}%")
...
def make_graph(df, ticker):
    ...
    line1, = ax1.plot(df['Year'], df["Dividends yield rate"], 'ro--', markersize=4, label='배당률', color='red') # 그래프1 그리기
    ...
    line2 = ax2.bar(df['Year'], df['Dividends'], label=ticker+"_배당금", color='blue') # 그래프2 그리기
...
def save_graph(): # 그래프 저장 위치 정하기
    filename = filedialog.asksaveasfilename(initialfile = 'Untitled.png',defaultextension=".png",filetypes=[("All Files","*.*"),("Portable Graphics Format","*.png")])
    plt.savefig(filename)

def save_excelfile(): # 엑셀 파일 저장 위치 정하기
    filename = filedialog.asksaveasfilename(initialfile = 'Untitled.xlsx',defaultextension=".xlsx",filetypes=[("All Files","*.*"),("Excel 통합 문서","*.xlsx")])
...
# 프레임 생성
frame_1 = Frame(master=window, relief=RAISED) # 티커 검색 버튼, 엑셀 저장 버튼, 그래프 저장 버튼 프레임
...
label_current_dividends_yield_rate = Label(master=frame_2, textvariable=current_dividends_yield_rate, font=gui_font)
...
button_search = Button(
    ...
    width=10,
    height=2,
    command=scraping_stock_data) # 검색 버튼
...
button_download_excel = Button(
    ...
    width=20,
    height=2,
    command=save_excelfile) #lambda: print("엑셀 파일이 저장되었습니다.")) # 엑셀 파일로 저장하는 버튼
...
button_download_graph = Button(
    ...
    width=20,
    height=2,
    command=save_graph) # 이미지 저장하는 버튼
